...
const ShowList = () => {
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      try {
        const response = await fetch('/api/drawresults');
        ...
        <table className="table-auto w-full border-collapse border border-gray-400">
          ...
          <tbody>
            {students.map((student) => (
              <tr key={student._id}>
                <td className="border border-gray-400 px-4 py-2">{student.drawDate}</td>
                <td className="border border-gray-400 px-4 py-2">{student.typeDigit}</td>
                <td className="border border-gray-400 px-4 py-2">{student.threeDigitOn}</td>
                <td className="border border-gray-400 px-4 py-2">{student.twoDigitOn}</td>
                <td className="border border-gray-400 px-4 py-2">{student.twoDigitLow}</td>
